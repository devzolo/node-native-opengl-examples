...
import { Benchmark, sleep, Texture } from 'utils';
...
async function initialize() {
  ...
  GL.enable(GL.BLEND);
  texture1 = await Texture.load(path.join('assets', 'textures', 'bloco.jpg'));
  texture2 = await Texture.load(path.join('assets', 'textures', 'grille.png'));
  texture3 = await Texture.load(path.join('assets', 'textures', 'font.bmp'));
}
